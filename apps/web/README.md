...
Copia `apps/web/.env.example` y rellena `VITE_PSP_BASE_URL` + `VITE_PSP_PUBLIC_KEY` para tokenizar contra el sandbox. El PAN nunca llega a Nest. Un refresh recupera el paso, la entrega y el `transactionId` desde `localStorage`; PAN/CVC/token no se persisten. `pnpm --filter @checkout/web test` exige cobertura **80%**.
